...
# How many words per Bedrock call. 600 words ≈ 800 tokens input, ~1200 output.
# Nova Lite is ~10x cheaper than Haiku so we can afford larger chunks,
# which gives the model more context and produces better-quality rewrites.
CHUNK_WORDS = 600
...
def _build_system_prompt(level: str, intent: str) -> str:
    ...
    Constructs the Bedrock system prompt by combining level + intent rules.
    System prompt is more token-efficient than putting instructions in the
    user message — Nova Lite respects system prompts reliably.
    """
...
def _call_bedrock(system_prompt: str, chunk: str) -> str:
    ...
    body = json.dumps({
        ...
        'messages': [{'role': 'user', 'content': [{'text': f'<chunk>\n{chunk}\n</chunk>'}]}],
        'inferenceConfig': {
            'maxTokens': 1500,
            'temperature': 0.7
        }
...
def _generate_annotations(text: str, level: str) -> list:
    ...
    type is one of: concept | formula | person | definition
    """
    sample = ' '.join(text.split()[:3000])  # increased from 2000 — Nova Lite is cheaper
